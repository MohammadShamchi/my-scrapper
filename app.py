...
# Import the actual Site2MD components
from src.site2md.cli.config import load_config
from src.site2md.crawl.crawler import Crawler
from src.site2md.utils.exceptions import Site2MDError
from src.site2md.utils.logging import get_logger, setup_logging
...
async def start_crawl(
    ...
    try:
        ...
        include_list = json.loads(
            include_patterns) if include_patterns != "[]" else []
        exclude_list = json.loads(
        ...
        if dry_run:
            ...
            try:
                ...
                raise HTTPException(
                    status_code=400, detail=f"Dry run failed: {str(e)}")
            finally:
